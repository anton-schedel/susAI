...
function App(): React.JSX.Element {
  ...
  const downloadTranscript = (transcript: string[]): void => {
    const content = transcript.map((t, i) => `[${i + 1}] ${t}`).join('\n\n')
    const blob = new Blob([`Interview Transcript\nGenerated by susAI on ${new Date().toLocaleString()}\n\n${'='.repeat(50)}\n\n${content}`], { type: 'text/plain' })
  ...
  const stopAndGetAssessment = async (): Promise<void> => {
    ...
    try {
      ...
      const data = (await response.json()) as { assessment: string; transcript: string[]; stats: object }
            ...
            <div className="assessment-header">
              ...
              {interviewTranscript && (
                <button
                  ...
                  onClick={() => downloadTranscript(interviewTranscript)}
